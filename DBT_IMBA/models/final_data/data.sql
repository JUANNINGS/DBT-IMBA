with feature_1 as (
    ...
    INNER JOIN {{ ref('user_features_2') }} F2 on F1.user_id = F2.user_id
),
...
feature_2 as (
    ...
    from feature_1 F3
    INNER JOIN {{ ref('up_features') }} UF on F3.user_id = UF.user_id
),
...
feature_3 as (
    ...
    from feature_2 F4
    INNER JOIN {{ ref('prd_features') }} PF on F4.product_id = PF.product_id
)
